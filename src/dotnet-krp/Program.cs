...
public class Program
{
    ...
    private static InMemoryLoggingProvider _logProvider;
    private static EndpointManager _endpointManager;
    
    public static async Task Main(string[] args)
    {
        var host = Host.CreateDefaultBuilder(args)
            .ConfigureLogging(loggingBuilder => loggingBuilder.AddKrpTerminalLogger())
        ...
        _endpointManager = host.Services.GetRequiredService<EndpointManager>();
        _logProvider = host.Services.GetRequiredService<InMemoryLoggingProvider>();

        await Task.WhenAll(host.RunAsync(), RunUiAsync());
    }
                    ...
                    try
                    {
                        ...
                        if (Console.KeyAvailable)
                        {
                            ...
                            switch (key.Key)
                            {
                                ...
                                case ConsoleKey.D1 when ctrl: _selectedTableIndex = 0; redraw = true; break;
                                case ConsoleKey.D2 when ctrl: _selectedTableIndex = 1; redraw = true; break;
...
#pragma warning restore CA1416
    ...
    private static Layout BuildLayout(string ctxName)
    {
    ...
    private static Panel BuildSelectedPanel()
    {
    ...
    private static Panel BuildLogsPanel()
    {
        var fixedRows = 3 + HEADER_SIZE;
        var rowsVis = Math.Max(1, Console.WindowHeight - fixedRows);

        // Tail: newest at bottom (follow when _selectedRowIndex == 0)
        var all = _logProvider!.ReadLogs(0, int.MaxValue).ToList();
        var total = all.Count;
        var follow = _selectedRowIndex == 0;

        var start = follow
            ? Math.Max(0, total - rowsVis)
            : Math.Max(0, total - _selectedRowIndex - rowsVis);

        var slice = all.Skip(start).Take(rowsVis).ToList();

        var tbl = new Table().NoBorder().Expand();

        // Fix widths of TIME/LEVEL, let MESSAGE flex and wrap.
        tbl.AddColumn(new TableColumn("[bold]TIME[/]") { NoWrap = true, Padding = new Padding(1) });
        tbl.AddColumn(new TableColumn("[bold]LEVEL[/]") {  NoWrap = true, Padding = new Padding(1) });
        tbl.AddColumn(new TableColumn("[bold]MESSAGE[/]") { NoWrap = false, Padding = new Padding(0) });

        foreach (var log in slice)
        {
            // Use Text for plain message (no markup evaluation).
            var msgText = new Text(log.Message ?? string.Empty);

            // Add pretty exception renderable under the message (no escaping).
            IRenderable msgCell = log.Exception is null
                ? msgText
                : new Rows(
                    msgText,
                    log.Exception.GetRenderable(new ExceptionSettings
                    {
    ...
    private static Panel BuildTablePanel()
    {
